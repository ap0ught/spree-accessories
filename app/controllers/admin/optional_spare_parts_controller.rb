class Admin::OptionalSparePartsController < Admin::BaseController
  resource_controller
  ...
  def available
    if params[:q].blank?
      ...
      if params[:type]=="product"
        @available_accessories = Product.find(:all,:include=>[:variants,:translations], :conditions => ['lower(name) LIKE ? OR lower(variants.sku) LIKE ?', "%#{params[:q].downcase}%", "%#{params[:q].downcase}%"])
  ...
  def remove
    ...
    @product.osps.delete(spare_part)
    render :layout => false
  end
